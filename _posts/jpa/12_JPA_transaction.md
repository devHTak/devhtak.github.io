...
- 보통 비즈니스 로직을 시작하는 서비스 계층에서 트랜잭션을 시작한다.
...
- 트랜잭션 설정하지 않을 때 영속성 컨텍스트
  - CUD
    - DB에 아무런 변경 사항도 저장되지 않는다
    - flush가 발생해야 db에 반영하는 데, 이는 트랜잭션 커밋 시점에 자동 발생
    - em.persist, em.remove 를 호출할 경우 예외(TransactionRequiredException) 발생 가능
  - Read
    - 트랜잭션 없이 읽기 동작. 요청 단위마다 영속성 컨텍스트 생성, 소멸
    - 1차 캐시, 지연 로딩 불가
