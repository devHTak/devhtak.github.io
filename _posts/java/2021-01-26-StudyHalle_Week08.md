...
#### 객체 지향의 세가지 관계
- IS A관계
  - 상속, 하위 개념이 상위 개념의 한 종류일 때
  - Cat is a animal
- HAS A 관계
  - 합성, 한 객체가 다른 객체를 부품, 필드로 소유할 때
  - Car has a engine
  - 굳이 인터페이스를 사용하지 않아도 되지만 인터페이스를 사용해 전략 패턴 같이 유연하게 사용 가능
- CAN DO 관계
  - 인터페이스, 객체가 어떤 할 수 있는 능력, 행동을 가질 때
  - Bird can fly. Plane can fly
  - Bird와 Plane 은 상속으로 묶을 연관겅은 없지만 날 수 있는 능력을 공유하므고 인터페아스로 묶을 수 있다
