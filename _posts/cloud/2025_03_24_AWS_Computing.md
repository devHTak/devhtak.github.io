...
### 스케일링
- IT 자원을 확장사거나 축소하는 기능
- 수직 스케일링(vertical scaling)
  - IT 자원의 용량(메모리, CPU 등)을 scale-up하거나 scale-down으로 분류
- 수평 스케일링(horizontal scaling)
  - IT 자원의 수량을 확장(scale-out)하거나 축소(scale-in)하는 것

#### AWS 오토 스케일링 서비스
- 클라우드 자원을 동적으로 확장하거나 축소하여 클라우드 자원의 규모를 조정하는 것
- EC2 인스턴스, ECS(Elastic Container Service), DynamoDB의 테이블 및 인덱스, Aurora 복제본 등이 대상이 된다
- Amazon EC2 오토 스케일링
  - EC2 인스턴스 내에 어플리케이션 워크로드를 동적으로 수용하도록 EC2 인스턴스의 수를 확장(scale out)하거나 축소(scale in)하는 것
  - 오토스케일링 그룹 내에 최대/최소 인스턴스 수, 스케일링 조정 정책에 따라 기준 설정
- Amazon EC2 오토 스케일링 구성 요소
  - 그룹: EC2 인스턴스를 오토 스케일링으로 조정하고 관리하기 위해 EC2 인스턴스를 논리적으로 구분하는 그룹으로 구성하며 최소 및 최대 인스턴스 수와 최초 요구하는 인스턴스 수 등 정보를 지정할 수 있다.
  - 구성 템플릿: EC2 인스턴스를 구성하는 템플릿으로 인스턴스 AMI, 인스턴스 유형, 키 페어, 보안 그룹 등 정보 지정
  - 조정 옵션: 오토 스케일링 그룹을 조정하는 다양한 방법 정의
- Amazon EC2 오토 스케일링의 인스턴스 수명 주기
  - 인스턴스 확장 (수동으로 인스턴스 수를 늘리거나 트래픽 증가에 따른 조정 정책이 적용되는 경우 또는 예약된 작업을 수행하는 경우)
    - 오토 스케일링에 할당된 구성 템플릿을 사용하여 필요한 수의 인스턴스 시작하며 '대기중' 상태
    - 완전히 구성되면 오토스케일링에 연결, 오토스케일링이 로드밸런서와 연결이 진행되고 '진행중' 상태로 전환
  - 인스턴스 축소 (수동으로 인스턴스 수를 줄이거나 트래픽 감소에 따른 조정 정책이 적용되는 경우 또는 예약된 작업을 수행하는 경우)
    - 하나 이상의 인스턴스를 종료한다. 이 때 오토스케일링은 종료 정책을 이용하여 종료할 인스턴스를 결정하고, '종료 중'으로 상태 전환
    - 완전히 종료되면오토 스케일링에서 삭제, 로드벨런서가 인스턴스의 등록 취소가 된 후에 '종료됨' 상태에 들어간다.
    - draining(graceful shutdown 수행) - 바로 삭제하지 않고 사용자 요청을 처리할 수 있도록 지정된 시간만큼 대기
- Amazon EC2 오토 스케일링 조정 옵션
  - 인스턴스를 일정한 수로 유지
    - 조정 옵션을 통해 지정된 수를 유지할 수 있다. 비정상 상태의 인스턴스가 있으면 종료하고 새로운 인스턴스를 시작하여 수를 유지
  - 수동 조정
    - 최대/최소 용량을 수동으로 변경할 수 있으며 업데이트한 사항에 맞춰 인스턴스를 생성하거나 종료
  - 동적 조정
    - 트래픽 변화에 따라 오토스케일링 그룹의 용량 조정
    - 대상 추적 조정: 대상 인스턴스를 추적하여 특정 지표의 목표값을 기준으로 유지하기 위해 오토스케일링 적용
    - 단계 조정: 용량을 단계별로 조정, 임곗값 기준으로 넘어서는 크기에 따라 단계적으로 조정
    - 단순 조정: 용량을 단일하게 조정하여 오토 스케일링 적용, 임곗값 기준으로 넘어서는 크기와 무관하게 단일하게 조정
  - 일정을 기반으로 예약된 조정
    - 작업 시간이나 날짜에 따라 오토스케일링 수행
  - 일정을 기반으로 예측 조정
    - 트래픽 흐름의 일일 및 주간 패턴을 예측하여 인스턴스 수를 늘리거나 줄일 수 있다.

### 인증/인가
- AWS 리소스 생성하고 관리하기
  - AWS 관리 콘솔
    - 웹 기반 사용자 인터페이스를 제공한다. 
  - AWS 명령줄 인터페이스(CLI)
    - 운영체제에 설치하면 shell 프로그램에서 AWS 서비스를 사용할 수 있다.
  - AWS 소프트웨어 개발 키트(SDK)
    - AWS 리소스를 프로그래밍적으로 사용하기 편리하도록 제공되는 라이브러리
- AWS API
  - 인증/인가
    - API를 이용하여 두 어플리케이션이 서로 통신하면서 정보를 주고받을 수 있는 것
    - 인증: 사용자가 적법한 서명 값을 가졌는지 확인
    - 인가: 인증받은 사용자가 API 권한을 수행할 수 있는지를 확인해야 한다.
    - 명세서: 요청할 때 규칙을 정리한 문서
  - AWS API: AWS 서비스를 사용하기 위해 도와주는 매개체로 예를들면 S3 버킷 생성 등에 사용

#### AWS IAM
- AWS IAM 이란
  - AWS 서비스와 리소스에 안전하게 접근할 수 있도록 관리하는 기능으로 '인증'과 '권한'을 통제한다.
- 구성 요소
  - 사용자, 그룹, 역할, 정책으로 구성된다.
  - 루트 사용자: 맨 처음 생성된 AWS 계정으로 모든 권한을 가지고 있다.
  - 사용자: 각 IAM 사용자는 자체 자격 증명을 보유한다. 특정 AWS 작업을 수행할 수 있게 권한을 통제할 수 있다.
  - 그룹: IAM 사용자 집합으로 그룹에 권한을 지정하여 다수의 사용자의 권한을 쉽게 관리할 수 있다.
  - 정책: 자격 증명이나 리소스와 연결될 때 요청을 허용하거나 거부할 수 있는 권한 정의
  - 역할: 특정 권한을 가진 계정에 생성할 수 있는 IAM 자격 증명
- 인증/인가 동작 방식
  - 인증: IAM 사용자가 AWS 리소스를 사용할 때 암호나 접근 키같은 자격 증명을 사용하여 인증받아야 한다.
  - 인가: IAM 사용자는 적합한 권한이 있는지 확인한다.

#### 출처
<img width="571" alt="image" src="https://github.com/user-attachments/assets/48df31c1-aa7e-4a51-82c5-e933d41323d1" />

- AWS 교과서(길벗) 중 1, 2, 8, 9장을 요약한 내용입니다.
