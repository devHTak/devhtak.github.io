...
- 순서 보장(event time)
  - Record 자체에 포함된 발생 시간(Event Time)을 기준으로 데이터를 처리하는 기법은 실시간 스트리밍 분석 및 순서 보장에 필수적인 접근 방식입니다.
  - Kafka 및 관련 생태계(Kafka Streams, Flink, Spark Streaming 등)에서 Event Time을 처리하는 구현 구조 및 패턴을 정리해 드립니다.
  - Event Time 메커니즘의 기본 개념
    - Kafka Record는 기본적으로 메시지 헤더/바디 외에도 자체 Record Timestamp 필드를 가지고 있습니다.
    - Record의 record.timestamp()를 직접 꺼내 비즈니스 로직에 활용할 수 있습니다.
  - Kafka Streams / Flink를 활용한 스트림 처리 (가장 권장)
    - Event Time 기반의 집계(Aggregation)나 지연 데이터(Late Data) 처리는 Kafka Streams나 Apache Flink 같은 전문 스트림즈 엔진을 사용할 때 가장 강력합니다.
  - 일반 Consumer에서의 로컬 버퍼링 (In-Memory Reordering)
    - 별도의 스트림 엔진 없이 일반 Kafka Consumer 애플리케이션에서 Event Time 순서를 정렬하고 싶을 때 사용하는 패턴
    - 시간 기반 버퍼(Priority Queue) 구현:
      - Consumer가 poll한 메시지를 즉시 DB에 넣지 않고, Event Time 기준 최소 힙(MㅔPriority Queue)에 담아둡니다.
    - Watermark / Delay 적용:
      - 현재 시각 기준 N초 전 데이터까지만 꺼내어 처리합니다.
  - DB Level 멱등성 및 버전 제어 (Eventual Consistency)
    - 메시지가 지연되어 순서가 뒤바뀌어 들어오더라도, 최종 결과(State)를 올바르게 보장하는 데이터베이스 처리 패턴입니다.
    - 최신 상태 덮어쓰기 방지 (Optimistic Locking / Timestamp Check):
      - DB Table에 last_event_time 컬럼을 둡니다.
      - last_event_time보다 **구버전(과거 시간)이면 무시(Discard)**하거나 별도 딜레이 큐로 이관합니다.
