...
- 멱등성 프로듀서
  - 기본 프로듀서의 동작 방식은 enable.idempotence 옵션을 사용하여 적어도 한번 전달(at least once delivery)를 지원
  - 프로듀서 PID와 시퀀스 넘버를 함께 전달하여 브로커는 여러번 요청이 오더라도 단 한번만 적재한다
  - enable.idempotence를 설정하면 retries 는 기본으로 Integer.MAX_VALUE로 설정되며 acks는 all(-1)로 설정된다
...
- 트랜잭선 프로듀서
  ...
  - 트랜잭션 프로듀서를 사용하기 위해서는 enable.idempotence true, transaction.id 임의의 String으로 정의, 컨슈머는 isolation.level read_commited 설정해야 한다
