...
- 컨슈머 오프셋 저장
  ...
  - key는 consumer group, topic, partition으로 구성
...
- 주키퍼
  ...
  - 분산 환경에서 상태관리와 동기화에 사용
    - 활성화/비활성화 브로커 관리, 컨트롤러 선출, 파티션 리더 선출, 메타데이터(토픽, 파티션 수 등) 관리
  - 클러스터 규모가 커지면서 병목현상 유발(성능 이슈, 느린 장애복구)
  - KRaft - 합의 알고리즘 기반 아키텍처 로 해결
